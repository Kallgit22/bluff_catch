...
class _GamePageState extends ConsumerState<GamePage> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(gameStateProvider, (previous, next) {
      if (next.lastEvent != AnimationEvent.none && (previous?.lastEvent != next.lastEvent || previous?.passCount != next.passCount)) {
    ...
    final activePlayer = gameState.players.isNotEmpty && gameState.activePlayerIndex >= 0 
          ...
          child: Stack(
            children: [
              ...
              if (activePlayer != null)
                Positioned(
                  right: 16, top: 80, bottom: 0,
                  child: _buildRightPanel(context, ref, gameState, uiState, activePlayer),
                ),

              // ZONE 6 - Top Opponents Row
              Positioned(
                top: 0, left: 180, right: 150,
              ...
              if (activePlayer != null)
                Positioned(
                  ...
                  child: Align(
                    ...
                    child: _buildActivePlayerHand(context, ref, gameState, uiState, activePlayer),
                  ),
                ),
            ],
          ),
  ...
  void _handleAnimationEvent(BuildContext context, WidgetRef ref, AnimationEvent event) {
    ...
    if (event == AnimationEvent.roundPassed || 
      ...
      if (gameState.status == GameStatus.playing && gameState.activePlayerIndex >= 0) {
  ...
  void _showExitConfirmation(BuildContext context, String playerName) {
    ExitConfirmationDialog.show(
      ...
      onConfirm: () {
        final activePlayer = ref.read(gameStateProvider).players.firstWhere((p) => p.currentTurn);
  ...
  Widget _buildMyProfile(BuildContext context, Player player) {
    ...
    final avatarColors = [
      ...
      Colors.indigoAccent, Colors.lightGreenAccent,
    ];
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(
                ' Cards',
                style: theme.textTheme.bodySmall?.copyWith(color: Colors.white70, fontSize: 10),
              ),
            ],
          ),
        ],
    ...
    for (int i = 1; i < N; i++) {
      ...
      opponents.add(
        Padding(
          ...
          child: Opacity(
            opacity: (player.hasPassed || player.hasPendingVictory || player.isEliminated) ? 0.5 : 1.0,
  ...
  Widget _buildLeaderboardPanel(BuildContext context, GameState gameState) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('LEADERBOARD', style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return Row(
                  children: [
                    Text(p.rankPosition != null ? '#' : '-', style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 12)),
                    const SizedBox(width: 8),
                    Expanded(child: Text(p.playerName, style: TextStyle(color: p.isEliminated ? Colors.grey : Colors.white, fontSize: 12), overflow: TextOverflow.ellipsis)),
                    Text('', style: const TextStyle(color: Colors.white70, fontSize: 10)),
                  ],
                );
              },
            ),
          ...
          children: [
                    ...
                    for (int i = 0; i < (poolSize > 8 ? 8 : poolSize); i++)
                      Positioned(
                        ...
                        child: Transform.rotate(
                          ...
                          child: const PlayingCardWidget(isHidden: true, width: 70, height: 105),
                        ),
                      ),
                  if (poolSize == 0)
                    Container(
                      ...
                      alignment: Alignment.center,
                      child: Text('EMPTY\nTABLE', textAlign: TextAlign.center, style: theme.textTheme.bodySmall?.copyWith(color: Colors.white.withValues(alpha: 0.3), fontWeight: FontWeight.bold)),
                    )
                  else
            ...
            if (poolSize > 0)
              Container(
                ...
                child: Text(' CARDS', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.white70, fontWeight: FontWeight.bold)),
              ),
          ],
        ),
      ),
  ...
  Widget _buildRightPanel(BuildContext context, WidgetRef ref, GameState gameState, GameUIState uiState, Player player) {
    ...
    final bool canChallenge = !isPoolEmpty && (gameState.lastPlayerId != null && gameState.lastPlayerId != player.playerId);
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            if (claim != null) ...[
              const Center(child: Text('ACTIVE CHAIN', style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1))),
            ...
            if (!isPoolEmpty) ...[
              FilledButton.icon(
                onPressed: () {
                  try {
                    ref.read(gameStateProvider.notifier).passTurn(player.playerId);
                    ref.read(gameUIProvider.notifier).resetSelection();
                  } catch (e) {
                ...
                label: const Text('Pass', style: TextStyle(fontWeight: FontWeight.bold)),
              ),
              const SizedBox(height: 12),
            ],
            FilledButton.icon(
              onPressed: hasSelected ? () => _handlePlayTap(context, ref, gameState, uiState, player) : null,
              style: FilledButton.styleFrom(
                backgroundColor: hasSelected ? Theme.of(context).colorScheme.primary : Colors.grey.withValues(alpha: 0.3),
            ...
            if (canChallenge) ...[
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: () => _handleChallengeTap(context, ref, gameState, uiState, player),
                style: FilledButton.styleFrom(
  ...
  Widget _buildActivePlayerHand(BuildContext context, WidgetRef ref, GameState gameState, GameUIState uiState, Player player) {
    final int cardCount = player.handCards.length;
    final double screenHeight = MediaQuery.of(context).size.height;
    
    final double handAreaHeight = math.max(100.0, math.min(150.0, screenHeight * 0.30));
    final double cardHeight = handAreaHeight * 0.75;
    final double cardWidth = cardHeight * 0.66;
    final double stride = cardWidth * 0.5;
    
    final double totalWidth = cardCount > 0 ? cardWidth + (cardCount - 1) * stride : 0;
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        physics: const BouncingScrollPhysics(),
        child: SizedBox(
          width: math.max(MediaQuery.of(context).size.width * 0.5, totalWidth + 32),
          child: Stack(
            alignment: Alignment.bottomCenter,
            clipBehavior: Clip.none,
            children: List.generate(cardCount, (index) {
              ...
              return Positioned(
                ...
                child: GestureDetector(
                  onTap: () => ref.read(gameUIProvider.notifier).toggleCardSelection(card),
  ...
  void _handlePlayTap(BuildContext context, WidgetRef ref, GameState gameState, GameUIState uiState, Player player) {
    if (gameState.pool.isEmpty) {
      RankSelectorBottomSheet.show(
        ...
        onRankSelected: (rank) {
          ...
          try {
            ...
            ref.read(gameStateProvider.notifier).throwCards(player.playerId, uiState.selectedCards, rank);
            ref.read(gameUIProvider.notifier).resetSelection();
          } catch (e) {
            _showErrorSnackBar(context, e.toString());
          }
      ...
      try {
        ref.read(audioServiceProvider).playCardThrow();
        ref.read(gameStateProvider.notifier).addCards(player.playerId, uiState.selectedCards, gameState.currentClaimedRank!);
        ref.read(gameUIProvider.notifier).resetSelection();
      } catch (e) {
        _showErrorSnackBar(context, e.toString());
      }
  ...
  Future<void> _handleChallengeTap(BuildContext context, WidgetRef ref, GameState gameState, GameUIState uiState, Player player) async {
    if (uiState.isResolvingChallenge) return;
    
    final previousPlayer = gameState.players.firstWhere((p) => p.playerId == gameState.lastPlayerId);
  ...
  void _showErrorSnackBar(BuildContext context, String message) {
    final cleanMessage = message.startsWith('Bad state: ') ? message.substring(11) : message;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
